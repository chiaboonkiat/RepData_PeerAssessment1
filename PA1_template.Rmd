...
```
Mean total number of steps taken per day: `r mean.total.steps`

```{r}
median.total.steps <- as.integer(median(activity.total$steps))
```
Median total number of steps taken per day: `r median.total.steps`
...
[2] The 5-minute interval containing the maximum number of average steps is: `r interval.max.steps`
...
```
[1] Total number of missing values in dataset: `r num.missing.values`
...
```
New mean total steps per day: `r new.mean.total.steps`
```{r}
new.median.total.steps<-as.integer(median(activity.filled.total$steps))
```
New median total steps per day: `r new.median.total.steps` 
